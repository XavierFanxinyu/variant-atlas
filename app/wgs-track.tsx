...
import FlagshipCaseLab from "./flagship-case-lab";
...
type Tab = "overview" | "course" | "workflow" | "cases" | "flagship" | "exam" | "standards";
type ExamLevel = keyof typeof wgsExamBanks;

const channels = ["SNV / indel", "Non-coding", "CNV", "Aneuploidy", "SV", "LOH / ROH", "STR", "mtDNA"];
const tabs: Array<[Tab, string]> = [["overview", "专项首页"], ["course", "28课课程"], ["workflow", "15步工作流"], ["cases", "18例矩阵"], ["flagship", "6个旗舰病例"], ["exam", "三级测验"], ["standards", "规范中心"]];
...
export default function WgsTrack() {
  ...
  const [caseAnswers, setCaseAnswers] = useState<Record<string, Array<number | null>>>({});
  ...
  const [examBest, setExamBest] = useState<Partial<Record<ExamLevel, number>>>({});

  useProgressPersistence("variant-atlas-wgs-track-v1", { tab, completed, revealed, workflow, caseScores, examBest, lessonId, caseId, caseStep, caseAnswers, examLevel, examAnswers, examResponses, examSubmitted }, saved => {
    setTab(saved.tab);
    setCompleted(saved.completed);
    setRevealed(saved.revealed);
    setWorkflow(saved.workflow);
    setCaseScores(saved.caseScores);
    setExamBest(saved.examBest);
    setLessonId(saved.lessonId);
    setCaseId(saved.caseId);
    setCaseStep(saved.caseStep);
    setCaseAnswers(saved.caseAnswers);
    setExamLevel(saved.examLevel);
    setExamAnswers(saved.examAnswers);
    setExamResponses(saved.examResponses);
    setExamSubmitted(saved.examSubmitted);
  });
  ...
  const currentCase = filteredCases.find((item) => item.id === caseId) ?? filteredCases[0];
  const categories = Array.from(new Set(wgsCases.map((item) => item.category)));
  const answered = (currentCase ? caseAnswers[currentCase.id] : undefined) ?? [];
  ...
  const wgsCertified = completed.length === wgsLessonCount && Object.values(caseScores).filter((score) => score >= 2).length >= 12 && (["W1","W2","W3"] as ExamLevel[]).every((level) => (examBest[level] ?? 0) >= 7);
  ...
  function answerCase(option: number) {
    if (!currentCase || answered[caseStep] != null) return;
    const next = [...answered]; next[caseStep] = option;
    setCaseAnswers((current) => ({ ...current, [currentCase.id]: next }));
    const score = completedQuizScore(currentCase.gates, next);
    if (score !== null) setCaseScores(current => ({ ...current, [currentCase.id]: Math.max(current[currentCase.id] ?? 0, score) }));
  }

  function changeFilter(field: "setting" | "family" | "category", value: string) {
    const nextFilters = { setting, family, category, [field]: value };
    const nextCase = wgsCases.find(item => (nextFilters.setting === "全部" || item.setting === nextFilters.setting) && (nextFilters.family === "全部" || item.family === nextFilters.family) && (nextFilters.category === "全部" || item.category === nextFilters.category));
    if (field === "setting") setSetting(value);
    if (field === "family") setFamily(value);
    if (field === "category") setCategory(value);
    if (nextCase) setCaseId(nextCase.id);
    setCaseStep(0);
  }
  function retryCase() {
    if (!currentCase) return;
    setCaseAnswers(value => ({ ...value, [currentCase.id]: [] })); setCaseStep(0);
  }
  ...
  return (
    <section className="wgs-track-page">
      ...
      {tab === "cases" && <section className="wgs-cases-section"><div className="section-heading"><div><span>CASE COVERAGE MATRIX</span><h2>18例现实边界情境</h2></div><p>通过检测场景、家系结构和变异通道交叉筛选；每例包含三道逐步揭示的病例关口。</p></div><div className="wgs-case-filters"><select value={setting} aria-label="检测场景" onChange={(event) => changeFilter("setting", event.target.value)}><option>全部</option><option>产前</option><option>产后</option></select><select value={family} aria-label="家系结构" onChange={(event) => changeFilter("family", event.target.value)}><option>全部</option><option>单人</option><option>三联体</option><option>扩展家系</option></select><select value={category} aria-label="变异通道" onChange={(event) => changeFilter("category", event.target.value)}><option>全部</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><span>{filteredCases.length}例匹配</span></div><div className="wgs-case-layout"><aside className="wgs-case-grid">{filteredCases.map((item) => <button key={item.id} className={item.id === currentCase?.id ? "active" : ""} onClick={() => selectCase(item)}><span>{item.id}{caseScores[item.id] !== undefined ? ` · ${caseScores[item.id]}/3` : ""}</span><b>{item.title}</b><small>{item.setting} · {item.family} · {item.channel}</small></button>)}</aside>{currentCase && currentGate ? <article className="wgs-case-workspace"><header><span>{currentCase.id} · {currentCase.category}</span><h2>{currentCase.title}</h2><p>{currentCase.summary}</p></header><div className="wgs-case-facts"><b>逐步揭示的病例资料</b>{currentCase.facts.map((fact, index) => <p key={fact}><i>{index + 1}</i>{fact}</p>)}</div><div className="wgs-case-boundary"><b>高风险边界</b><p>{currentCase.boundary}</p></div><div className="wgs-gate"><span>CASE GATE {caseStep + 1}/3</span><h3>{currentGate.question}</h3><div>{currentGate.options.map((option, index) => { const selected = answered[caseStep] === index; const shown = answered[caseStep] != null; return <button key={option} disabled={shown} className={`${selected ? "selected" : ""} ${shown && index === currentGate.answer ? "correct" : ""}`} onClick={() => answerCase(index)}>{String.fromCharCode(65 + index)}. {option}</button>; })}</div>{answered[caseStep] != null && <p className={answered[caseStep] === currentGate.answer ? "correct" : "incorrect"}><b>{answered[caseStep] === currentGate.answer ? "判断正确" : "需要修正"}</b>{currentGate.rationale}</p>}<footer><button className="secondary" disabled={caseStep === 0} onClick={() => setCaseStep((step) => step - 1)}>上一关</button>{caseStep < 2 ? <button className="primary" disabled={answered[caseStep] == null} onClick={() => setCaseStep((step) => step + 1)}>下一关 →</button> : <button className="primary" disabled={completedQuizScore(currentCase.gates, answered) === null} onClick={retryCase}>重新作答（保留最高分）</button>}</footer></div><SourceLinks keys={currentCase.sourceKeys} /></article> : <div className="wgs-empty" role="status"><h3>暂无匹配情境</h3><p>当前筛选组合还没有收录病例。不会显示不匹配的其他病例。</p><button className="secondary" onClick={() => { setSetting("全部"); setFamily("全部"); setCategory("全部"); setCaseStep(0); }}>清除筛选</button></div>}</div></section>}

      {tab === "flagship" && <FlagshipCaseLab />}

      {tab === "exam" && <section className="wgs-exam-section"><div className="section-heading"><div><span>WGS COMPETENCY CHECK</span><h2>三级专项测验</h2></div><p>W1检查通道与术语，W2检查专项规则路由，W3检查复杂病例整合。每级8题，达到7/8视为通过，最高分写入统一学习档案。</p></div><div className="wgs-exam-levels">{(["W1", "W2", "W3"] as ExamLevel[]).map((level) => <button key={level} className={examLevel === level ? "active" : ""} onClick={() => { setExamLevel(level); setExamAnswers({}); setExamSubmitted(false); }}><b>{level}</b><span>{level === "W1" ? "基础路由" : level === "W2" ? "专项判断" : "病例整合"}</span><small>最高 {examBest[level] ?? 0}/8</small></button>)}</div><div className="wgs-exam-layout"><div>{exam.map((item, questionIndex) => <article className="wgs-exam-question" key={item.id}><header><span>{String(questionIndex + 1).padStart(2, "0")} · {item.tag}</span><h3>{item.q}</h3></header>{item.options.map((option, index) => <button key={option} className={`${examAnswers[item.id] === index ? "selected" : ""} ${examSubmitted && index === item.answer ? "correct" : ""}`} disabled={examSubmitted} onClick={() => setExamAnswers((answers) => ({ ...answers, [item.id]: index }))}>{String.fromCharCode(65 + index)}. {option}</button>)}{examSubmitted && <p className={examAnswers[item.id] === item.answer ? "correct" : "incorrect"}>{item.rationale}</p>}</article>)}</div><aside><span>{examLevel}专项结果</span><strong>{examSubmitted ? examScore : Object.keys(examAnswers).length}<small>/8</small></strong><p>{examSubmitted ? examScore >= 7 ? "本级通过。请继续下一级，或回到病例矩阵巩固边界。" : "本级尚未通过。按错题标签返回对应课程复习。" : "完成全部8题后提交；答案只保存在当前浏览器。"}</p><button className="primary" disabled={Object.keys(examAnswers).length < exam.length || examSubmitted} onClick={() => { setExamSubmitted(true); setExamResponses(value => ({ ...value, ...examAnswers })); setExamBest((current) => ({ ...current, [examLevel]: Math.max(current[examLevel] ?? 0, examScore) })); }}>提交本级测验</button>{examSubmitted && <button className="secondary" onClick={() => { setExamAnswers({}); setExamSubmitted(false); }}>重新作答</button>}</aside></div></section>}
