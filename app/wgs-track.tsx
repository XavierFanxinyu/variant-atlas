...
type StoredState = { completed?: string[]; revealed?: string[]; workflow?: string[]; caseScores?: Record<string, number>; examBest?: Partial<Record<ExamLevel, number>>; lessonId?: string };
...
export default function WgsTrack() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved.examBest) setExamBest(saved.examBest);
  ...
  useEffect(() => {
    window.localStorage.setItem("variant-atlas-wgs-track-v1", JSON.stringify({ completed, revealed, workflow, caseScores, examBest, lessonId }));
    notifyLearningProgress();
  }, [completed, revealed, workflow, caseScores, examBest, lessonId]);
